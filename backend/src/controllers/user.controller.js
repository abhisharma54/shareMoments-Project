...
const registerUser = AsyncHandler( async(req, res) => {
    ...
    if(existedUser) throw new ApiError(404, "User already exists with this email or username")
...
const loginUser = AsyncHandler( async(req,res) => {
    ...
    if(!user) throw new ApiError(401, "Unauthorized request, please provide authentice email or username")

    const isPasswordValid = await user.isPasswordCorrect(password);
    if(!isPasswordValid) throw new ApiError(402, "Password is Incorrect")

    const loggedInUser = await userModel.findById(user._id).select("-password -refreshToken")
    if(!loggedInUser) throw new ApiError(500, "Something went wrong while login the user, please try again!");
...
const updateAccountDetails = AsyncHandler( async(req, res) => {
    ...
    if(!updatedUser) throw new ApiError(500, "Failed to update the account details");
...
const changePassword = AsyncHandler( async(req, res) => {
    ...
    if(confirmPassword?.trim() === "") throw new ApiError(402, "Confirm Password is required")
...
const newRefreshedAccessToken = AsyncHandler( async(req, res) => {
    ...
    try {
        const verifiedToken = jwt.verify(incomingRefreshToken, process.env.REFRESH_TOKEN);
        ...
        const { newRefreshedToken, accessToken } = await generateAccessAndRefreshToken(user._id);
        ...
        .json(new ApiResponse(200, {accessToken, refreshToken: newRefreshedToken}, "Access Token Refreshed"))
    } catch (error) {
...
const uploadAvatar = AsyncHandler( async(req, res) => {
    ...
    if(!uploadedAvatar.url) throw new ApiError(500, "Failed to upload avatar file on cloudinary");
    ...
    return res
    .status(200)
    .json(new ApiResponse(200, uploadAvatar.avatar, "Avatar uploaded or updated successfully"))
})

// upload and update cover image
const uploadCoverImage = AsyncHandler( async(req, res) => {
    const coverImageFilePath = req.file.path;
    if(!coverImageFilePath) throw new ApiError(400, "CoverImage file is required");

    const uploadedCoverImage = await uploadOnCloudinary(coverImageFilePath, "coverImage");
    if(!uploadedCoverImage) throw new ApiError(500, "Failed to upload coverImage file on cloudinary");
    ...
    if(!uploadCoverImage) throw new ApiError(500, "Failed to upload or update coverImage file on cloudinary")
    ...
    return res
    .status(200)
    .json(new ApiResponse(200, uploadCoverImage.coverImage, "CoverImage uploaded or updated successfully"))
})
...
const getUserProfile = AsyncHandler( async(req, res) => {
     ...
     return res 
    .status(200)
    .json(new ApiResponse(200, profile[0], "User channel fetched successfully"))
})
...
const getAllPosts = AsyncHandler( async(req, res) => {
    ...
    if(!user) throw new ApiError(500, "Failed to  get all posts")

    return res
    .status(200)
    .json(new ApiResponse(200, user[0].allPosts, "All Posts fetched successfully"))
})
